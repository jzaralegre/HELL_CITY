...
public MyHttpResponse(playerid, response_code, data[])
{
	...
	{
		format(string, 256, "[PRC Notice]: %s(%d) has joined the server.", name, playerid);
	...
	if(response_code == 200)
	{
		...
		if(data[0] == 'N')
		{
			format(string, 256, "[PRC Notice]: %s(%d) thank you for joining!", name, playerid);
